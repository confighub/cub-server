...
// OS clipboard support, for commands that hand the caller a secret to paste
// rather than echo it to the terminal -- here `cub server keycloak open`, which
// puts Keycloak's admin password on the clipboard.
//
// Lifted from the cub CLI's own clipboard.go, which does this for
// `cub cluster open`. Copied rather than imported because that file is in the
// CLI's package main and cannot be imported; both are ConfigHub's own MIT code.
// If one of them learns something the other should, they have to be told twice.
//
// We shell out to the platform's clipboard tool instead of taking a clipboard
// library dependency: this installer already shells out to kubectl and docker,
// and the X11/Wayland paths would otherwise pull cgo into the build. Plenty of
// environments have no clipboard at all (containers, ssh sessions, CI), so
// copying is best-effort and every caller must handle the error by falling back
